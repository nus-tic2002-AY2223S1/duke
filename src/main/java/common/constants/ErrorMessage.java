package common.constants;

public class ErrorMessage {
    public static final String DUPLICATED_TASK_ERROR_MSG = "Task is duplicated. Please enter a new task.";

    public static final String EMPTY_TASK_LIST_ERROR_MSG = "Task list is empty.";

    public static final String INVALID_FILE_ERROR_MSG = "Invalid file i/o.";

    public static final String INVALID_TASK_DESCRIPTION_ERROR_MSG = "Invalid task description!";

    public static final String MARKED_TASK_ERROR_MSG = "Task has already been marked.";

    public static final String NOT_EXIST_TASK_ERROR_MSG = "Task does not exist in the task list.";

    public static final String UNMARKED_TASK_ERROR_MSG = "Task has already been unmarked.";

    public static final String EXCEPTION_ERROR_MSG = "Caught exception: %s";
}
